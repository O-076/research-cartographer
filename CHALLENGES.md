...
**Approach:**
- After every delta update, call `simulation.alpha(0.3).restart()` — not `1.0`
...
**Approach:**
- Check the [IQ Series GitHub repo](https://github.com/microsoft/iq-series) for working examples
- Ask on [Discord](https://aka.ms/agentsleague/discord) — Microsoft product teams are active there
- If A2A proves too complex before Day 4, fall back to a simple `asyncio` task chain — the architecture still works, just without the A2A layer
- Document the fallback clearly in the README if used

**Do NOT:** Spend more than half of Day 4 fighting A2A setup — fall back early if needed.
...
- This is normal and expected — don't treat it as an error
...
- Use real papers from the same domain (not random ones) — real contradictions and gaps will appear organically
